...
async function removeNotification({ authUser, notificationId }) {
  const currentUser = await getCurrentUser(authUser);
...
async function removeAllNotifications({ authUser }) {
  const currentUser = await getCurrentUser(authUser);
  await Notification.destroy({ where: { recipient_id: currentUser.id } });
  return { ok: true };
}
