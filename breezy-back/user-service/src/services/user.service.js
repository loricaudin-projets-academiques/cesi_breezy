...
async function updateMe({ authUser, updates: rawUpdates }) {
  ...
  if (updates.name && updates.name.length > 80) {
    throw createHttpError(400, "Le nom ne peut pas depasser 80 caracteres.");
  }

  if (updates.note && updates.note.length > 10000) {
    throw createHttpError(400, "La note ne peut pas depasser 10000 caracteres.");
  }

  if (typeof rawUpdates.isPrivate === "boolean") {
    updates.isPrivate = rawUpdates.isPrivate;
  }
  ...
  if (rawUpdates.music && typeof rawUpdates.music === "object") {
    updates.music = {
      ...currentUser.music,
      ...rawUpdates.music,
    };
...
async function followUser({ authUser, username }) {
  ...
  if (targetUser.id === currentUser.id) {
    throw createHttpError(400, "Impossible de te suivre toi-meme.");
  }
