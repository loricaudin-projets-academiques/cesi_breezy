...
export class MockFeedService implements IFeedService {
  ...
  async createRemotePost(payload: { content: string; category: PostCategory; image?: string; video?: string }): Promise<Post> {
    ...
    const post = this.createPost(author, payload.content, payload.category, payload.image, payload.video);
