/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import { motion } from 'motion/react';
import { Zap, Menu, Edit2, Music, Gamepad2, ImageIcon, Link as LinkIcon, Sparkles } from 'lucide-react';
import { UserProfile, Post, ProfileStatType, ProfileSubTab } from '../types';
import { INITIAL_FOLLOWERS } from '../mockData';
import SpotifyWidget from '../components/SpotifyWidget';
import PostCard, { PostInteractionHandlers, PostListState } from '../components/PostCard';
import { getAvatarUrl } from '../components/Avatar';
import { playTick, playChime } from '../audio';

interface ProfileScreenProps extends PostInteractionHandlers, PostListState {
  user: UserProfile;
  posts: Post[];
  activeProfileSubTab: ProfileSubTab;
  setActiveProfileSubTab: (tab: ProfileSubTab) => void;
  onOpenHamburger: () => void;
  onOpenPostModal: () => void;
  onOpenBioEditor: () => void;
  onOpenNoteEditor: () => void;
  onOpenAvatarSelector: () => void;
  onOpenStatsModal: (type: ProfileStatType) => void;
  onMusicPlayToggle: () => void;
  onMusicChange: (updates: Partial<UserProfile['music']>) => void;
}

// Les onglets internes du profil, dans leur ordre d'affichage
const PROFILE_SUB_TABS: ProfileSubTab[] = ['posts', 'followers', 'following', 'friends'];

export default function ProfileScreen({
  user,
  posts,
  activeProfileSubTab,
  setActiveProfileSubTab,
  onOpenHamburger,
  onOpenPostModal,
  onOpenBioEditor,
  onOpenNoteEditor,
  onOpenAvatarSelector,
  onOpenStatsModal,
  onMusicPlayToggle,
  onMusicChange,
  postComments,
  commentDrafts,
  showCommentsForPost,
  onToggleStar,
  onToggleLike,
  onToggleComments,
  onCommentDraftChange,
  onAddComment,
  triggerToast
}: ProfileScreenProps) {
  // On ne montre que les posts publiés par l'utilisateur connecté
  const userPosts = posts.filter(p => p.authorUsername === user.username);
  return (
    <motion.div
      key="profile-screen"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: -10 }}
      className="p-4 flex flex-col gap-4 text-left"
    >
      {/* En-tête avec le titre et le bouton menu */}
      <div className="flex justify-between items-center bg-transparent shrink-0">
        <div className="flex items-center gap-1">
          <Zap className="w-3.5 h-3.5 text-breezy-neon active-nav-glow" />
          <span className="text-[10px] font-mono tracking-widest text-breezy-neon uppercase select-none">
            Mon profil
          </span>
        </div>
        {/* Bouton pour ouvrir le menu latéral */}
        <button
          onClick={onOpenHamburger}
          className="w-10 h-10 rounded-xl glassmorphism-light hover:bg-breezy-card flex items-center justify-center text-breezy-icy cursor-pointer active:scale-95 transition shadow-sm border border-breezy-border"
          title="Menu"
        >
          <Menu className="w-4.5 h-4.5 text-breezy-icy" />
        </button>
      </div>

      {/* SECTION PRINCIPALE DU PROFIL — mise en page 2 colonnes */}
      <div className="grid grid-cols-12 gap-3 pb-2 select-none">
        {/* COLONNE GAUCHE (7/12) : Nom, bio, statistiques */}
        <div className="col-span-8 min-w-0 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start gap-1">
              <div>
                <h2 className="text-[20px] font-sans font-bold text-breezy-icy leading-none tracking-tight">
                  {user.name}
                </h2>
                <p className="text-xs font-sans text-purple-300 font-medium mt-1">{user.username}</p>
              </div>
              {/* Petit crayon pour modifier la bio */}
              <button
                onClick={onOpenBioEditor}
                className="p-1 rounded-full bg-breezy-card hover:bg-breezy-card-hover text-breezy-muted hover:text-breezy-neon select-none"
                title="Modifier la bio"
              >
                <Edit2 className="w-3 h-3" />
              </button>
            </div>

            {/* La bio — cliquable pour l'éditer directement */}
            <div
              onClick={onOpenBioEditor}
              className="glass mt-3 w-full max-w-full min-h-[72px] rounded-[24px] border border-breezy-border px-4 py-3 text-[11px] leading-relaxed text-breezy-icy shadow-[0_4px_12px_rgba(0,0,0,0.3)] transition duration-300 hover:border-breezy-border-active cursor-pointer font-sans"
              title="Cliquer pour modifier..."
            >
              <p className="max-w-full whitespace-pre-wrap break-words [overflow-wrap:anywhere] [word-break:break-word]">
              {user.bio ? `"${user.bio}"` : <span className="text-breezy-muted italic">Clique pour écrire ta bio...</span>}
              </p>
            </div>
          </div>

          {/* Boutons statistiques (Abonnés, Abonnements, Amis) — ouvrent la liste en modal */}
          <div className="flex flex-col gap-1.5 mt-4 select-none w-full">
            <div className="flex gap-1.5 w-full">
              <button
                onClick={() => onOpenStatsModal('followers')}
                className={`flex-1 glass p-2 rounded-xl text-center transition duration-300 active:scale-95 ${
                  activeProfileSubTab === 'followers' ? 'border-breezy-lavender bg-breezy-card' : 'hover:border-breezy-border-active'
                }`}
              >
                <div className="text-xs font-bold text-breezy-neon">{user.followers}</div>
                <div className="text-[8px] text-breezy-muted font-semibold uppercase tracking-wider font-sans">Abonnés</div>
              </button>

              <button
                onClick={() => onOpenStatsModal('following')}
                className={`flex-1 glass p-2 rounded-xl text-center transition duration-300 active:scale-95 ${
                  activeProfileSubTab === 'following' ? 'border-breezy-lavender bg-breezy-card' : 'hover:border-breezy-border-active'
                }`}
              >
                <div className="text-xs font-bold text-breezy-lavender">{user.following}</div>
                <div className="text-[8px] text-breezy-muted font-semibold uppercase tracking-wider font-sans">Abonnements</div>
              </button>
            </div>

            <button
              onClick={() => onOpenStatsModal('friends')}
              className={`w-full glass p-2 rounded-xl text-center transition duration-300 active:scale-95 flex items-center justify-between px-3 ${
                activeProfileSubTab === 'friends' ? 'border-breezy-purple bg-breezy-card' : 'hover:border-breezy-border-active'
              }`}
            >
              <span className="text-[8px] text-breezy-muted font-semibold uppercase tracking-wider font-sans">Amis</span>
              <span className="text-xs font-bold text-breezy-purple">{user.friends}</span>
            </button>
          </div>
        </div>

        {/* COLONNE DROITE (5/12) : Photo de profil, note, liens sociaux */}
        <div className="col-span-4 min-w-0 flex flex-col items-center gap-2.5">
          {/* Avatar avec halo lumineux */}
          <div className="relative group text-center flex flex-col items-center">
            <div className="relative">
              {/* Cercle de dégradé qui pulse autour de l'avatar */}
              <div className="absolute inset-x-0 bottom-0 top-0 rounded-full bg-gradient-to-tr from-[#E1306C] via-purple-500 to-[#AEEBFF] blur-md opacity-35 animate-pulse" />

              <button
                onClick={onOpenAvatarSelector}
                className="relative block w-[92px] h-[92px] rounded-full overflow-hidden p-[3px] bg-gradient-to-tr from-[#E1306C] via-purple-500 to-[#AEEBFF] shadow-[0_0_15px_rgba(174,235,255,0.3)] transition duration-300 transform active:scale-95 z-10"
                title="Changer de photo"
              >
                <div className="w-full h-full rounded-full overflow-hidden bg-breezy-bg flex items-center justify-center">
                  {/* On passe par le helper centralisé pour gérer le fallback Dicebear */}
                  <img
                    src={getAvatarUrl(user.avatar, user.username, user.name)}
                    className="w-full h-full object-cover rounded-full"
                    alt="Photo de profil"
                  />
                </div>
                {/* Overlay "MODIFIER" au survol */}
                <div className="absolute inset-0 bg-breezy-overlay opacity-0 group-hover:opacity-100 flex items-center justify-center text-[8px] font-mono font-bold transition rounded-full text-breezy-icy z-20">
                  MODIFIER
                </div>
              </button>
            </div>

            {/* Bulle de statut flottante (la note d'humeur) */}
            <button
              onClick={onOpenNoteEditor}
              className="absolute -top-1 -right-4 glass-bright border border-breezy-lavender rounded-2xl p-1.5 shadow-[0_4px_12px_rgba(0,0,0,0.6)] cursor-pointer hover:border-breezy-neon transition max-w-[105px] z-20 text-[9px] text-left leading-tight transform hover:-translate-y-0.5 text-breezy-icy"
              title="Modifier ta note"
            >
              <p className="font-mono text-[7px] text-breezy-neon font-black uppercase tracking-widest -mb-0.5">Note</p>
              <p className="truncate">{user.note || <span className="text-breezy-muted italic">Ajouter...</span>}</p>
            </button>
          </div>

          {/* Bouton rapide pour modifier la note d'humeur */}
          <button
            onClick={onOpenNoteEditor}
            className="w-full glass rounded-full p-2 py-1 border border-breezy-border hover:border-breezy-border-active transition cursor-pointer text-center text-[10px] text-breezy-muted hover:text-breezy-icy font-sans flex items-center justify-center gap-1 leading-none shadow-sm"
          >
            <span className="w-1 h-1 rounded-full bg-purple-400 animate-ping" />
            note...
          </button>

          {/* Raccourci pour la musique */}
          <button
            onClick={onMusicPlayToggle}
            className="w-full border border-pink-500/30 bg-pink-500/5 text-pink-300 hover:bg-pink-500/15 rounded-full px-2.5 py-1 text-[9.5px] font-medium flex items-center justify-center gap-1 shadow-[0_0_10px_rgba(255,102,196,0.15)] transition duration-300 active:scale-95"
          >
            <Music
              className={`w-3.5 h-3.5 text-pink-400 ${user.music.isPlaying ? 'animate-spin' : ''}`}
              style={{ animationDuration: '6s' }}
            />
            <span className="truncate max-w-[70px] leading-none">
              {user.music.title ? user.music.title : "Configurer la musique"}
            </span>
          </button>

          {/* Grille de liens vers les réseaux sociaux */}
          <div className="grid grid-cols-2 gap-2 mt-1 w-[84px] mx-auto select-none">
            <button
              onClick={() => {
                playChime();
                triggerToast("Ouverture LinkedIn...");
              }}
              className="w-[38px] h-[38px] rounded-full bg-breezy-bg border border-breezy-border hover:border-breezy-border-active flex items-center justify-center text-breezy-muted hover:text-breezy-lavender hover:bg-breezy-card transition shadow-md active:scale-95 cursor-pointer"
              title="LinkedIn"
            >
              in
            </button>

            <button
              onClick={() => {
                playChime();
                triggerToast("Ouverture Facebook...");
              }}
              className="w-[38px] h-[38px] rounded-full bg-breezy-bg border border-breezy-border hover:border-breezy-border-active flex items-center justify-center text-breezy-muted hover:text-breezy-lavender hover:bg-breezy-card transition shadow-md active:scale-95 cursor-pointer"
              title="Facebook"
            >
              f
            </button>

            <button
              onClick={() => {
                playChime();
                triggerToast("Ouverture X (Twitter)...");
              }}
              className="w-[38px] h-[38px] rounded-full bg-breezy-bg border border-breezy-border hover:border-breezy-lavender/50 flex items-center justify-center text-breezy-icy hover:text-breezy-lavender hover:bg-breezy-card transition shadow-md hover:shadow-[0_0_12px_rgba(139,92,246,0.25)] active:scale-95 cursor-pointer flex items-center select-none"
              title="X (Twitter)"
            >
              <span className="flex items-center justify-center w-full h-full text-breezy-muted hover:text-breezy-icy">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </span>
            </button>

            <button
              onClick={() => {
                playChime();
                triggerToast("Ouverture Discord...");
              }}
              className="w-[38px] h-[38px] rounded-full bg-breezy-bg border border-breezy-border hover:border-breezy-lavender/50 flex items-center justify-center text-breezy-icy hover:text-breezy-lavender hover:bg-breezy-card transition shadow-md hover:shadow-[0_0_12px_rgba(139,92,246,0.25)] active:scale-95 cursor-pointer"
              title="Discord"
            >
              <Gamepad2 className="w-4 h-4" />
            </button>
          </div>
        </div>
      </div>

      {/* ONGLETS INTERNES : Posts, Abonnés, Abonnements, Amis */}
      <div className="flex gap-4 border-b border-breezy-border pb-1.5 pt-1 select-none">
        {PROFILE_SUB_TABS.map((sub) => {
          const isActive = activeProfileSubTab === sub;
          return (
            <button
              key={sub}
              onClick={() => {
                playTick();
                setActiveProfileSubTab(sub);
              }}
              className={`text-[11px] font-sans font-medium transition-all duration-300 relative capitalize tracking-wider ${
                isActive ? 'text-breezy-lavender font-black' : 'text-breezy-muted hover:text-breezy-icy'
              }`}
            >
              {sub}
              {/* Ligne de soulignement qui glisse sous l'onglet actif */}
              {isActive && (
                <motion.div
                  layoutId="profileSubTabUnderline"
                  className="absolute -bottom-1.5 left-0 right-0 h-0.5 bg-gradient-to-r from-breezy-lavender to-breezy-neon rounded-full"
                />
              )}
            </button>
          );
        })}
      </div>

      {/* CONTENU DE L'ONGLET SÉLECTIONNÉ */}
      {activeProfileSubTab === 'posts' ? (
        <>
          {/* Invite à créer un post */}
          <button
            onClick={onOpenPostModal}
            className="w-full glass-bright border-breezy-border hover:border-breezy-border-active text-breezy-icy rounded-2xl p-4 cursor-pointer text-left transition duration-300 flex items-center justify-between group relative"
          >
            <span className="text-xs text-breezy-icy group-hover:text-breezy-icy font-sans font-medium">
              Quoi de neuf ?
            </span>
            <div className="flex items-center gap-3 text-breezy-muted group-hover:text-breezy-icy transition">
              <ImageIcon className="w-3.5 h-3.5 hover:text-breezy-neon" />
              <LinkIcon className="w-3.5 h-3.5 hover:text-breezy-lavender" />
              <Sparkles className="w-3.5 h-3.5 hover:text-breezy-purple active-nav-glow animate-pulse" />
            </div>
          </button>

          {/* Les posts publiés par l'utilisateur */}
          <div className="flex flex-col gap-2.5 mt-1 font-sans">
            <div className="px-0.5 flex justify-between items-baseline mb-1">
              <h5 className="text-[10px] font-mono tracking-widest text-breezy-faint uppercase select-none">
                Mes publications
              </h5>
              <span className="text-[9px] font-mono text-breezy-neon select-none">Synchronisé</span>
            </div>

            {userPosts.length === 0 ? (
              <div className="py-10 text-center text-breezy-faint text-xs bg-breezy-card rounded-2xl border border-breezy-border">
                Aucune publication pour l'instant.
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {userPosts.map((post) => (
                  <PostCard
                    key={post.id}
                    post={post}
                    comments={postComments[post.id]}
                    commentDraft={commentDrafts[post.id]}
                    showComments={showCommentsForPost[post.id]}
                    onToggleStar={onToggleStar}
                    onToggleLike={onToggleLike}
                    onToggleComments={onToggleComments}
                    onCommentDraftChange={onCommentDraftChange}
                    onAddComment={onAddComment}
                    triggerToast={triggerToast}
                  />
                ))}
              </div>
            )}
          </div>
        </>
      ) : (
        /* Onglets abonnés/abonnements/amis (liste simulée pour l'instant) */
        <div className="flex flex-col gap-2 mt-1">
          <div className="px-0.5 flex justify-between items-baseline mb-1">
            <h5 className="text-[10px] font-mono tracking-widest text-breezy-faint uppercase select-none">
              {activeProfileSubTab === 'followers' && "Mes abonnés"}
              {activeProfileSubTab === 'following' && "Mes abonnements"}
              {activeProfileSubTab === 'friends' && "Amis proches"}
            </h5>
          </div>
          {INITIAL_FOLLOWERS.filter((f) => {
            if (activeProfileSubTab === 'friends') return f.followsMe && f.followedByMe;
            if (activeProfileSubTab === 'following') return f.followedByMe;
            return true; // tous pour "followers"
          }).map((follower) => (
            <div
              key={follower.username}
              className="glass rounded-[20px] p-3 flex items-center justify-between border border-breezy-border shadow-md text-left"
            >
              <div className="flex items-center gap-2.5">
                <img src={getAvatarUrl(follower.avatar, follower.username, follower.name)} className="w-8 h-8 rounded-full object-cover" alt={follower.name} />
                <div>
                  <h4 className="text-xs font-semibold text-breezy-icy leading-none">{follower.name}</h4>
                  <p className="text-[10px] text-breezy-muted mt-0.5">{follower.username}</p>
                </div>
              </div>
              <button
                onClick={() => {
                  playChime();
                  triggerToast(`Action effectuée pour ${follower.name}`);
                }}
                className="px-2.5 py-1 rounded-full text-[9px] font-bold bg-breezy-neon/20 text-breezy-neon border border-breezy-neon/30 hover:bg-breezy-neon/30 transition"
              >
                Message
              </button>
            </div>
          ))}
        </div>
      )}

      {/* Widget Spotify en bas du profil */}
      <div className="mt-2 text-left">
        <SpotifyWidget
          music={user.music}
          onChangeMusic={onMusicChange}
          triggerToast={triggerToast}
        />
      </div>
    </motion.div>
  );
}
