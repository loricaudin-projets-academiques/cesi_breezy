...
export default function SearchScreen({
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        const { data } = await api.get<Follower[]>('/users/search', {
          params: { q: query },
        });
  ...
  const handleToggleFollow = async (user: Follower) => {
    ...
    try {
      ...
      setSearchedUsers((prev) => prev.map((item) => (
        item.username === user.username ? data : item
      )));
      ...
      {!hasQuery ? (
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <div className="flex flex-col gap-2 text-left">
            <p className="text-[9px] font-mono tracking-wider text-white/35 uppercase">
            ...
            {searchedUsers.map((user) => (
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] font-sans font-bold text-breezy-icy truncate">
                      {user.name}
                      {user.isFriend && (
                        <span className="ml-1 text-[8px] font-mono text-breezy-neon uppercase">ami</span>
                      )}
                    </p>
                ...
                >
                  {user.followedByMe ? (
                    <span className="text-[#AEEBFF] flex items-center gap-1">
