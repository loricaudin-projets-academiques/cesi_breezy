...
export function useFeed(
  ...
  const [commentImageDrafts, setCommentImageDrafts] = useState<Record<string, string>>({});
  ...
  const handleCommentImageChange = (postId: string, url?: string) => {
    setCommentImageDrafts((prev) => ({ ...prev, [postId]: url ?? '' }));
  };

  const handleAddComment = async (postId: string) => {
    const draft = commentDrafts[postId]?.trim() ?? '';
    const image = commentImageDrafts[postId] || undefined;
    if (!draft && !image) return;

    playChime();

    try {
      const newComment = await feedService.addComment(postId, draft, image);
      ...
      setCommentImageDrafts((prev) => ({ ...prev, [postId]: '' }));
