...
export function useFeed(
  ...
  useEffect(() => {
    ...
    async function loadFeed() {
      try {
        ...
        if (!cancelled) {
          triggerToast(getErrorMessage(error, t("toasts.feedLoadError")));
        }
      }
    }
  ...
  }, [currentUser.username, triggerToast, t]);
  ...
  const handleToggleLike = async (postId: string) => {
    ...
    try {
      ...
      if (updatedPost.likedByUser) {
        triggerToast(t("toasts.likeAdded", { name: target.authorName }));
      }
    } catch (error) {
      triggerToast(getErrorMessage(error, t("toasts.likeError")));
    }
  };
  ...
  const handleToggleStar = async (postId: string) => {
    ...
    try {
      ...
      setPosts((prev) => prev.map((post) => (post.id === postId ? updatedPost : post)));
      triggerToast(updatedPost.starredByUser ? t("toasts.starAdded") : t("toasts.starRemoved"));
    } catch (error) {
      triggerToast(getErrorMessage(error, t("toasts.starError")));
    }
  };
  ...
  const handleAddPost = async (content: string, category: PostCategory, image?: string) => {
    try {
      ...
      triggerToast(t("toasts.postAdded", { category }));
    } catch (error) {
      triggerToast(getErrorMessage(error, t("toasts.postError")));
    }
  };
  ...
  const handleAddComment = async (postId: string) => {
    ...
    try {
      ...
      triggerToast(t("toasts.commentAdded"));
    } catch (error) {
      triggerToast(getErrorMessage(error, t("toasts.commentError")));
    }
  };
