...
export default function SpotifyWidget({ music, onChangeMusic, triggerToast }: SpotifyWidgetProps) {
  ...
  const handlePlayToggle = () => {
    ...
    triggerToast(music.isPlaying ? t('toasts.musicPaused') : t('toasts.musicPlaying', { title: music.title }));
  };
  ...
  const handleSaveConfig = (e: React.FormEvent) => {
    ...
    setShowConfig(false);
    triggerToast(t('toasts.musicUpdated'));
  };
        ...
        <form onSubmit={handleSaveConfig} className="flex flex-col gap-2.5 text-left">
          ...
          <div className="flex flex-col gap-0.5">
            <input
              ...
              placeholder={t('music.coverPlaceholder')}
              className="w-full text-xs rounded-xl bg-white/[0.03] p-2.5 px-3 text-breezy-icy placeholder-white/20 border border-white/5 focus:outline-none focus:border-breezy-border-active transition"
            />
          </div>
          <button
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left">
              <h4 className="text-[12px] font-sans font-semibold text-breezy-icy/70 tracking-tight">{t('music.noMusic')}</h4>
              <p className="text-[10px] font-sans text-white/30 -mt-0.5">{t('music.noMusicDesc')}</p>
            </div>
          </div>
          <button
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <p className="text-[11px] font-mono tracking-wider text-white/40 uppercase">{t('music.nowPlaying')}</p>
          </div>
