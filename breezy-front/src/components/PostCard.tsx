    ...
    >
      ...
      {post.image && (
        <div className="relative h-44 rounded-xl overflow-hidden border border-white/10 mt-1">
          <img src={post.image} className="w-full h-full object-cover" alt={t('post.imageAlt')} />
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-2">
            <span className="text-[8.5px] font-mono text-white/60 bg-black/40 px-1.5 py-0.5 rounded border border-white/10">
              {t('post.imageLabel')}
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/[0.04] mt-0.5 font-mono select-none">
        ...
        <button
          onClick={() => {
            ...
            triggerToast(t('toasts.linkCopied'));
          }}
